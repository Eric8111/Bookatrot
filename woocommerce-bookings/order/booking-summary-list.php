...
?>
	...
	<!-- Show extra customer requirement -->
	<?php 
	echo '<strong class="wc-booking-summary-number">' . __('Horse requirement:'). '</strong>'; 
	$horse_details = get_post_meta( $booking->get_id(), '_horse_details_by_customer', true );
	if( !empty($horse_details) ){
		$horse_details = json_decode($horse_details); 
		?>
		<ul class="wc-booking-summary-list">
			<?php foreach ($horse_details as $counter => $value) { ?>
				<li> <strong><?php _e('Person #') ?><?php echo $counter+1; ?></strong> Weight:<?php echo $value->weight; ?> Height: <?php echo $value->height; ?></li>
			<?php } ?>
		</ul>
	<?php } ?>
